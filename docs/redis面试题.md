...
个人学习的话，你可以自己本机安装 Redis 或者通过 Redis 官网提供的[在线 Redis 环境open in new window](https://try.redis.io/)来实际体验 Redis。
...
### 13. 购物车信息用 String 还是 Hash 存储更好呢?
...
![hash-shopping-cart.b8c84954](https://img.ggball.top/picGo/hash-shopping-cart.b8c84954.png)
...
### 15. 使用 Set 实现抽奖系统需要用到什么命令？

+   `SPOP key count` ： 随机移除并获取指定集合中一个或多个元素，适合不允许重复中奖的场景。
+   `SRANDMEMBER key count` : 随机获取指定集合中指定数量的元素，适合允许重复中奖的场景。

### 16. 使用 Bitmap 统计活跃用户怎么做？
...
![文件事件处理器](https://img.ggball.top/picGo/redis-event-handler.png)

相关阅读：[Redis 事件机制详解open in new window](http://remcarpediem.net/article/1aa2da89/) 。

### 19. Redis6.0 之前为什么不使用多线程？
...
相关阅读：[为什么 Redis 选择单线程模型open in new window](https://draveness.me/whys-the-design-redis-single-thread/) 。

### 20. Redis6.0 之后为何引入了多线程？
...
![AOF 记录日志过程](https://javaguide.cn/assets/redis-aof-write-log-disc.ca22bd65.png)
...
+   [rdb\_bigkeysopen in new window](https://github.com/weiyanwei412/rdb_bigkeys) : Go 语言写的用来分析 Redis 的 RDB 快照文件用的工具，性能更好。
...
![加入布隆过滤器之后的缓存处理流程图](https://img.ggball.top/picGo/redis-cache-penetration-bloom-filter.png)
